...
GlobalScroll.Scroller.prototype = {
	...
	Draw : function () {
		var sHTML = "";
		sHTML += '<div id="LAY_'+this.sName+'_Parent" style="position:relative; width:100%; padding:0px; margin: 0px; height:'+(this.nHeight * this.nViewCnt)+'px; overflow:hidden;" OnMouseOver="'+this.sName+'.bStop=true; " OnMouseOut="'+this.sName+'.bStop=false;">';
		for (var i=0; i<this.aItem.length; i++) {
			sHTML	+= '<div style="position: absolute; border:1px solid red; padding: 0px; margin: 0px; top:'+(this.nHeight * i) + 'px; width:100%; height:'+this.nHeight+'px;" id="' + this.sDivLayID + i + '">' + this.aItem[i] + '</div>';
		}
		sHTML += '</div>';
