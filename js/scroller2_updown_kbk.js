...
GlobalScroll.Scroller.prototype = {
	...
	Down : function () {
		if (this.nTotHei > 1) {
			if (this.bStop) {
				if (this.sScrollArrow != 'down') this.Redraw('down');
				...
				for (i = 0; i<this.nTotHei; i++) {
					...
					if (parseInt(this.oTmp.style.top) <= -this.nHeight) this.oTmp.style.top = this.nHeight * (this.nTotHei - 1);
				}
	...
	Up : function () {
		if (this.nTotHei > 1) {
			if (this.bStop) {
				if (this.sScrollArrow != 'up') this.Redraw('up');
	...
	Draw : function () {
		var sHTML = "";
		sHTML += '<div id="LAY_'+this.sName+'_Parent" style="width:100%; height:'+(this.nHeight * this.nViewCnt)+'px; position:relative; overflow:hidden;" OnMouseOver="'+this.sName+'.bStop=false; " OnMouseOut="'+this.sName+'.bStop=true;">';
	...
	Redraw : function (dir) {
		if (dir == 'up') {
			for (var i = 0; i < this.nTotHei; i++) {
				this.oTmp = document.getElementById(this.sDivLayID + i);
				if (parseInt(this.oTmp.style.top) >= this.nHeight) {
					this.oTmp.style.top = parseInt(this.oTmp.style.top) - (this.nHeight * this.nTotHei);
				}
			}
		} else if (dir == 'down') {
			for (var i = 0; i < this.nTotHei; i++) {
