...
(function($){
	var Fn = autocomplete = function(el, options) {
		...
		var selectors = {
					akcKeywordFrame	: null,		// 자동완성 IFRAME,
		...
		this.iframesrc	= "/globalData/searchAKC/akcNew2.php";
		...
		akcLayerCreate : function () {
			...
			var postionObj = $(">SPAN:first", el);
				...
				.bind({
					...
					"blur"		: function(){ self.AKCFocusOut() },
					"load"		: function(){
									sel.akcKeywordLayer = $(this).contents().find("DIV#akcLayer");
									sel.akcLayerContent = $(this).contents().find("DIV#akcLayer>UL#akcLayerContent");
			...
			if(e.which == 40) {
				if(opt.akcListCurrentLine < opt.akcListTotalLine) {
					...
					if(opt.akcListCurrentLine > 0) {
						$("#akcSearchResult_"+opt.akcListCurrentLine, sel.akcLayerContent).removeClass("select");
					}

					opt.akcListCurrentLine++;
					$("#akcSearchResult_"+opt.akcListCurrentLine, sel.akcLayerContent).addClass("select");

					// 키워드 입력
					$(sel.akcKeywordInput).val( $("#akcSearchResult_"+opt.akcListCurrentLine, sel.akcLayerContent).attr('text') );
				}
		...
		makeAKCData : function() {
			...
			$.ajax({
				...
				success : function(data, textStatus) {
					self.akcSearchResult(data);
					sel.akcKeywordFrame.contentWindow.resizeIframe(sel.akcKeywordFrame);
				},
